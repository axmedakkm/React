...
function QtyStepper({ value, onChange }) {
  ...
  return (
    <Box className="flex items-center border border-gray-300 rounded-lg overflow-hidden flex-1">
      <IconButton
        onClick={() => {
          handleAddToCart(item); // Маҳсулотро ба LocalStorage меандозад
          navigate("/shop");     // Бе reload кардан ба саҳифаи Shop мегузарад
        }}
        sx={{
          background: "#287FE8",
          color: "#fff",
...
function InfoModal({ item, open, onClose }) {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
          ...
          {item.unit && (
            <Chip
              label={t("catalog.unit", { unit: item.unit })}
              size="small"
              variant="outlined"
            />
          )}
...
export default function CatalogPage() {
  ...
  const handleAddToCart = (item) => {
    ...
    setSnackbar({ open: true, msg: t("catalog.addedToCart", { name: item.name }) });
  };

  return (
    <Box className="max-w-[1200px] mx-auto p-4">
      <Box className="flex justify-between items-center mb-6">
        <Typography variant="h5" fontWeight="bold">{t("catalog.title")}</Typography>
        <Button variant="contained" sx={{ backgroundColor: "#287FE8" }} onClick={() => getProducts(page)}>
          {t("catalog.refresh")}
        </Button>
      </Box>
      ...
      <Box className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {loading
          ...
          : products.map((item) => (
            <Card key={item.id} elevation={0} sx={{ borderRadius: "14px", border: "1px solid #e5e7eb", display: "flex", flexDirection: "column", height: "100%" }}>
              <Box sx={{ position: "relative", aspectRatio: "4/3", background: "#f9f9f9" }}>
                <img src={item.image} alt={item.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.src = "https://placehold.co/300x220?text=No+Image"; }} />
                <IconButton size="small" onClick={() => setInfoItem(item)} sx={{ position: "absolute", top: 6, right: 6, background: "rgba(255,255,255,0.85)" }}>
                  <InfoOutlinedIcon fontSize="small" />
                </IconButton>
              </Box>
              <CardContent sx={{ flex: 1, display: "flex", flexDirection: "column", p: "12px !important" }}>
                <Typography sx={{ fontSize: "12.5px", mb: 1 }}>{item.name}</Typography>
                <Typography sx={{ color: "#287FE8", fontWeight: "bold", fontSize: "18px", mt: "auto", mb: 1 }}>
                  {item.price?.toLocaleString("ru-RU")} ₽/{item.unit}
                </Typography>
                <Box className="flex items-center gap-2">
                  <QtyStepper value={qty[item.id] || 1} onChange={(v) => setQty((prev) => ({ ...prev, [item.id]: v }))} />
                  <IconButton onClick={() => handleAddToCart(item)} sx={{ background: "#287FE8", color: "#fff", borderRadius: "8px", width: 42, height: 36 }}>
                    <ShoppingCartIcon fontSize="small" />
                  </IconButton>
                </Box>
              </CardContent>
      ...
      <InfoModal item={infoItem} open={Boolean(infoItem)} onClose={() => setInfoItem(null)} />
      ...
      <Snackbar open={snackbar.open} autoHideDuration={2500} onClose={() => setSnackbar({ ...snackbar, open: false })} anchorOrigin={{ vertical: "bottom", horizontal: "center" }}>
        <Alert severity="success" variant="filled" onClose={() => setSnackbar({ ...snackbar, open: false })} sx={{ width: "100%" }}>{snackbar.msg}</Alert>
      </Snackbar>
    </Box>
  );
}